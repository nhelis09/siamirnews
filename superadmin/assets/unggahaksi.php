...
$id =   $_POST['idberita'] . '.jpg'; //mengambil nama file dari url parameter
$size = $_FILES['berkas']['size']; //mengambil ukuran file
$type = $_FILES['berkas']['type']; //mengambil tipe file
$folder = "../../fotoberita/"; //Folder untuk menampung berkas. Pastikan Anda telah membuatnya
// upload Process
//1 Megabyte = 1000000
if ($size <= 1000000 and $type == 'image/jpeg') {   //Melakukan validasi tipe file dan ukuran file 5 MB
    ...
    header("location:../berita.php?pesan=fileterkirim&name=$name&size=$size&type=$type");
} else {
    header("location:../berita.php?pesan=filegagalterkirim&name=$name&size=$size&type=$type");
}
